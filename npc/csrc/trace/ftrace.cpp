...
static void load_symbols(const char *path) {//从ELF符号表提取函数信息，后续用于call/ret地址反查函数名
  ...
  int fd = open(path, O_RDONLY);//打开ELF文件
  if (fd < 0) {
    return;
  }

  struct stat st;//获取文件元信息
  if (fstat(fd, &st) < 0) {
    close(fd);
    return;
  }
  ...
  void *buf = mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0);//映射ELF到内存
  ...
  if (eh->e_ident[EI_MAG0] != ELFMAG0 || eh->e_ident[EI_MAG1] != ELFMAG1 ||
      eh->e_ident[EI_MAG2] != ELFMAG2 || eh->e_ident[EI_MAG3] != ELFMAG3) {//检查ELF
...
void ftrace_log(uint64_t pc, uint64_t target_pc, int is_call) {
  ...
  if (is_call) {
    ...
    const FuncSym *f = find_by_pc(pc32);//返回时按PC反查
    if (f) {
